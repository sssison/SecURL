<<<<<<<< HEAD:02_feature-engineering/feature-generation/lexical-content-features/feature_generation_content_function.py
...
def get_html(url):
    try:
        html = get(url, timeout=100)
        html = html.text if html else None
    except:
        html = None
    return html

def get_word_freq_dict(url):
    html = get_html(url)
    if html:
...
def word_count(url):
    word_freq = get_word_freq_dict(url)
    total = 0
    if word_freq:
        for i in word_freq:
            total += word_freq[i]
...
if __name__ == '__main__':
    ...
    print(div_tag_count(url))
    print(has_log_in_html(url))
...
if __name__ == '__main__':
    ...
    print(sus_js_count(url))
>>>>>>>> 17bc40e1efb346db6e9434c9850b20d80ce4073d:02_feature-engineering/feature_generation_content_function_urlin.py
